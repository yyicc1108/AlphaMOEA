...
def data_generator(instance, run_num, NP=100, n_gen=100):
    ...
    for seed in range(run_num):
        ...
        pd.DataFrame(res_agemoea.F).to_csv(
            "../results/Part-1-2/dtlz/agemoea_" + instance + "_g" + str(n_gen) + '_PF_' + str(seed + 1) + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    #
    # print(df_zdt)
    # df_zdt.to_csv("ZDT-agemoea-Eva-Running-Times.csv", index=False)
    ...
    print(df_dtlz)
    df_dtlz.to_csv("DTLZ-Agemoea-Eva-Running-Times.csv", index=False)
