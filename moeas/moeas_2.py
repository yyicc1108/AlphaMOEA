...
def data_generator(instance, run_num, NP=200, n_gen=100):
    ...
    for seed in range(run_num):
        ...
        if instance[:4] == 'dtlz':
            ...
            pd.DataFrame(res_rvea.F).to_csv(
                "../results/dtlz/rvea_" + instance + "_g" + str(n_gen) + '_PF_' + str(seed + 1) + '.csv',  index=False)
        else:
            pd.DataFrame(res_nsga2.F).to_csv(
...
if __name__ == '__main__':
    ...
    #
    # print(df_dtlz)
    # df_dtlz.to_csv("../results/times/DTLZ-MOEAs-running-times-1.csv", index=False)
    ...
    for instance in zdt_instances:
        ...
        print("Instance: {0}, \t\t Running times: {1}s ".format(row, time.time() - s_time))

    print(df_zdt)
    df_zdt.to_csv("../results/times/ZDT-MOEAs-running-times-2.csv", index=False)
