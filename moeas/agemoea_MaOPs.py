...
def data_generator(instance, run_num, NP=100, n_gen=100, n_obj=4):

    problem = get_problem(instance, n_obj=n_obj)
    ...
    for seed in range(run_num):
        ...
        pd.DataFrame(res_agemoea.F).to_csv(
            "../results/Part-3/dtlz/agemoea_" + instance + "_g" + str(n_gen) + '_PF_M' + str(n_obj) + "_" + str(seed + 1) + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    for instance in dtlz_instances:
        for obj in objs:
            s_time = time.time()
            row = [instance + "-" + str(obj)]
            for n_gen in g_nens:
                times = data_generator(instance, run_num, NP=100, n_gen=n_gen, n_obj=obj)
                row.extend([times])
                print(row)
    ...
    print(df_dtlz)
    df_dtlz.to_csv("DTLZ-Agemoea-MaOPs-Running-Times.csv", index=False)
