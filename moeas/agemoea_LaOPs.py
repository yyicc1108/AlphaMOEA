...
def data_generator(instance, run_num, NP=100, n_gen=100, n_var=50):
    ...
    for seed in range(run_num):
        ...
        pd.DataFrame(res_nsga2.F).to_csv(
            "../results/Part-4/dtlz/nsga2_" + instance + "_g" + str(n_gen) + '_PF_D_' + str(n_var) + "_" + str(seed + 1) + '.csv', index=False)

        pd.DataFrame(res_rvea.F).to_csv(
            "../results/Part-4/dtlz/rvea_" + instance + "_g" + str(n_gen) + '_PF_D_' + str(n_var) + "_" + str(seed + 1) + '.csv', index=False)

        pd.DataFrame(res_moead.F).to_csv(
            "../results/Part-4/dtlz/moead_" + instance + "_g" + str(n_gen) + '_PF_D_' + str(n_var) + "_" + str(seed + 1) + '.csv', index=False)

        pd.DataFrame(res_agemoea.F).to_csv(
            "../results/Part-4/dtlz/agemoea_" + instance + "_g" + str(n_gen) + '_PF_D' + str(n_var) + "_" + str(seed + 1) + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    print(df_dtlz)
    df_dtlz.to_csv("DTLZ-Agemoea-LaOPs-Running-Times-1.csv", index=False)
