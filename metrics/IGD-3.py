...
def get_metric(instance, moeas, scaler, igd_ind):
    ...
    if "alphaEA" in moeas:
        ...
        for file in PF_files:
            ...
            alpha_igd_list.append(igd_ind(_pf))
    ...
    for moea in moeas:
        if moea != "alphaEA":
            ...
            for file in PF_files:
                pf = pd.read_csv(file, header=0)
                _pf = scaler.transform(np.array(pf))
                igd.append(igd_ind(_pf))

            stat, p = wilcoxon(igd, alpha_igd_list)
            sig = '+/-' if p < 0.05 else '='

            igd_list.append(round(np.mean(igd), 10))
            igd_list.append(round(np.std(igd), 10))
            igd_list.append(sig)

    igd_list.append(round(np.mean(alpha_igd_list), 10))
    igd_list.append(round(np.std(alpha_igd_list), 10))

    #print(instance, '\t', igd_list)
    return igd_list
...
def main(instances):
    ...
    for index, instance in enumerate(instances):
        ...
        igd_ind = IGD(problem.ReferObjV)
        scaler = get_all_PFs_scaler(instance)
        igd_list = get_metric(instance, moeas, scaler, igd_ind)

        igd_list.insert(3, round(moea_times.iloc[index, 1], 2))
        igd_list.insert(7, round(moea_times.iloc[index, 2], 2))
        igd_list.insert(11, round(moea_times.iloc[index, 3], 2))
        igd_list.append(round(alpha_times.iloc[index, 1], 2))

        row.extend(igd_list)
        print(instance, '\t', row)
        df.loc[len(df)] = row

    save_name = instance[:-1]+"_igd-1.csv" if instance[0] == 'z' else instance[:-1]+"_igd-2.csv"
